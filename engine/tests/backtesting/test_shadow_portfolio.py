...
class TestShadowPortfolio:
    def test_record_snapshot(self):
        ...
        positions = [
            ShadowPosition(
                ticker="AAPL", weight=0.5, price=175.0, composite_score=82.0
            ),
            ShadowPosition(
                ticker="MSFT", weight=0.5, price=410.0, composite_score=78.0
            ),
        ]
        portfolio.record_snapshot(
            date(2026, 2, 24), positions, portfolio_value=1_000_000.0
        )
    ...
    def test_cannot_backfill(self):
        ...
        positions = [
            ShadowPosition(
                ticker="AAPL", weight=1.0, price=175.0, composite_score=82.0
            )
        ]
        portfolio.record_snapshot(
            date(2026, 2, 25), positions, portfolio_value=1_000_000.0
        )

        with pytest.raises(ValueError, match="Cannot backfill"):
            portfolio.record_snapshot(
                date(2026, 2, 24), positions, portfolio_value=999_000.0
            )
    ...
    def test_performance_calculation(self):
        ...
        portfolio.record_snapshot(
            date(2026, 2, 25), positions, portfolio_value=1_010_000.0
        )
        portfolio.record_snapshot(
            date(2026, 2, 26), positions, portfolio_value=1_005_000.0
        )
