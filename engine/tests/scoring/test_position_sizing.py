...
class TestAsymmetryTiers:
    ...
    def test_asymmetry_above_5x_max_20(self):
        """Asymmetry > 5x -> max 20%."""
        result = compute_position_size(6.0, CompositeTier.EXCEPTIONAL)
        assert result == pytest.approx(20.0)

    def test_asymmetry_3_to_5x_max_10(self):
        """Asymmetry 3-5x -> max 10%."""
        result = compute_position_size(4.0, CompositeTier.EXCEPTIONAL)
        assert result == pytest.approx(10.0)

    def test_asymmetry_1_5_to_3x_max_5(self):
        """Asymmetry 1.5-3x -> max 5%."""
        result = compute_position_size(2.0, CompositeTier.EXCEPTIONAL)
        assert result == pytest.approx(5.0)

    def test_asymmetry_below_1_5x_max_3(self):
        """Asymmetry < 1.5x -> max 3%."""
        result = compute_position_size(1.2, CompositeTier.EXCEPTIONAL)
...
class TestConvictionScaling:
    ...
    def test_high_60_pct_of_max(self):
        """High: 60% of max."""
        result = compute_position_size(6.0, CompositeTier.HIGH)
        assert result == pytest.approx(12.0)

    def test_medium_30_pct_of_max(self):
        """Medium: 30% of max."""
        result = compute_position_size(6.0, CompositeTier.MEDIUM)
        assert result == pytest.approx(6.0)

    def test_none_conviction_zero(self):
        """NONE conviction -> 0% position."""
        result = compute_position_size(6.0, CompositeTier.NONE)
...
class TestCombinedScenarios:
    ...
    def test_moderate_asymmetry_high_conviction(self):
        ...
        result = compute_position_size(4.0, CompositeTier.HIGH)
        assert result == pytest.approx(6.0)

    def test_low_asymmetry_medium(self):
        """Asymmetry 1.0x (max=3%) + Medium (30%) -> 0.9%."""
        result = compute_position_size(1.0, CompositeTier.MEDIUM)
        assert result == pytest.approx(0.9)

    def test_medium_asymmetry_exceptional(self):
        """Asymmetry 2.5x (max=5%) + Exceptional (100%) -> 5.0%."""
        result = compute_position_size(2.5, CompositeTier.EXCEPTIONAL)
        assert result == pytest.approx(5.0)
