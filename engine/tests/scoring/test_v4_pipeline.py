...
class TestScoreUniverseV4:
    ...
    def test_beta_none_falls_back_to_sector_wacc(self):
        ...
        data_no_beta = [_make_ticker_data("AAPL")]
        assert data_no_beta[0].beta is None
        results = score_universe_v4(data_no_beta, shiller_cape=25.0)
        assert len(results) == 1
    ...
    def test_beta_provided_uses_company_wacc(self):
        ...
        td_no_beta = _make_ticker_data("AAPL")
        td_with_beta = _make_ticker_data("AAPL")
        td_with_beta.beta = 1.5  # high beta => higher WACC => different scores

        results_no_beta = score_universe_v4([td_no_beta], shiller_cape=25.0)
        results_with_beta = score_universe_v4([td_with_beta], shiller_cape=25.0)
