...
def upgrade() -> None:
    ...
    sa.Column('track_c', sa.JSON().with_variant(postgresql.JSONB(astext_type=sa.Text()), 'postgresql'), nullable=True),
    ...
    op.create_index('ix_v4_scores_asset_scored', 'v4_scores', ['asset_id', 'scored_at'], unique=False)
    op.create_index('ix_v4_scores_scored_at', 'v4_scores', ['scored_at'], unique=False)

    # Add new columns to ml_model_runs
    op.add_column('ml_model_runs', sa.Column('model_qualifies', sa.Boolean(), nullable=False, server_default=sa.text('false')))
