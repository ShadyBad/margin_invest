...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE users SET subscription_plan = 'portfolio' WHERE subscription_plan = 'operator'"
    )
    op.execute(
        "UPDATE users SET subscription_plan = 'institutional' WHERE subscription_plan = 'allocator'"
    )
    op.execute(
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE users SET subscription_plan = 'operator' WHERE subscription_plan = 'portfolio'"
    )
    op.execute(
        "UPDATE users SET subscription_plan = 'allocator' WHERE subscription_plan = 'institutional'"
    )
    op.execute(
