...
export function ChartTooltip({ active, payload, label }: ChartTooltipProps) {
  ...
  return (
    <div className="bg-bg-elevated/95 backdrop-blur-[8px] border border-border-subtle rounded-lg px-3 py-2 shadow-lg">
      ...
      {data.delta != null && (
        <p className={`text-xs font-mono ${data.delta >= 0 ? "text-emerald-400" : "text-red-400"}`}>
          {data.delta > 0 ? "+" : ""}{data.delta.toFixed(1)} pts
        </p>
      )}
      <div className="flex items-center gap-2 mt-0.5">
        {data.signal && (
          <span className="text-xs font-mono text-text-secondary uppercase">{data.signal}</span>
        )}
        {data.conviction && (
          <span className="text-xs font-mono text-text-tertiary uppercase">{data.conviction}</span>
        )}
      </div>
    </div>
  )
