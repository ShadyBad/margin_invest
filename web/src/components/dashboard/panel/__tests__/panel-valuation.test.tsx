...
describe("PanelValuation", () => {
  ...
  it("renders empty state when no methods and no marginInvestValue", () => {
    render(<PanelValuation {...baseProps} marginInvestValue={null} methods={{}} />)
    expect(screen.getByText("No valuation data")).toBeInTheDocument()
  })

  it("renders price ladder when buyPrice and sellPrice provided", () => {
    render(<PanelValuation {...baseProps} buyPrice={22.0} sellPrice={35.0} />)
  ...
  it("does not render price ladder when buyPrice is null", () => {
    render(<PanelValuation {...baseProps} buyPrice={null} sellPrice={35.0} />)
    expect(screen.queryByText("Buy Zone")).not.toBeInTheDocument()
  })

  it("does not render price ladder when sellPrice is null", () => {
    render(<PanelValuation {...baseProps} buyPrice={22.0} sellPrice={null} />)
    expect(screen.queryByText("Buy Zone")).not.toBeInTheDocument()
  })

  it("renders valuation methods even without marginInvestValue", () => {
    render(<PanelValuation {...baseProps} marginInvestValue={null} />)
    expect(screen.getByText("DCF Model")).toBeInTheDocument()
