---
title: "Composite Score & Tracks"
description: "How the dual-track system evaluates stocks as Compounders, Mispricings, or Both — and determines composite tiers."
order: 3
...
A moat durability score of 3 or 4 is required for EXCEPTIONAL composite tier. A score of 2 is the minimum for HIGH.
</TechnicalDetail>
...
- HIGH composite tier threshold: compounding power > 0.08
- EXCEPTIONAL composite tier threshold: compounding power > 0.15
...
- HIGH composite tier threshold: growth gap > 0.03
- EXCEPTIONAL composite tier threshold: growth gap > 0.08
...
For EXCEPTIONAL composite tier, all four methods must converge (not just three), and the discount must exceed 50%.
</TechnicalDetail>
...
For EXCEPTIONAL composite tier, catalyst strength must exceed the 80th percentile, meaning the strongest signal is in the top 20%.
...
<TechnicalDetail summary="Detailed composite tier thresholds">
...
- **When both tracks qualify**, the engine takes the strongest composite tier from either track. A stock earning HIGH on Track A and WATCHLIST on Track B is classified as HIGH composite tier with opportunity type "Both."
</TechnicalDetail>
