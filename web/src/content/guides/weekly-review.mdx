...
A consistent weekly review process helps you stay on top of scoring changes without over-trading on daily noise. The scoring pipeline updates regularly, but meaningful conviction changes happen on a weekly cadence. Here's a structured approach.
...
- **Conviction changes** — upgrades or downgrades in conviction level (e.g., HIGH to EXCEPTIONAL, or EXCEPTIONAL to WATCHLIST). These are meaningful signals.
...
- Which stocks moved up or down in conviction level?
- Open the asset detail page to see which factors drove the change
- A change in conviction level (e.g., HIGH to EXCEPTIONAL) is meaningful
- A small change in composite score within the same conviction level is usually noise

Focus on conviction-level transitions, not point-by-point score movements. The scoring pipeline produces continuous scores, but the conviction level is the actionable signal.
...
<Callout type="tip">
Consistency matters more than frequency. A disciplined weekly review is more effective than checking the dashboard five times a day. The scoring pipeline is designed to surface meaningful changes — trust the conviction levels and avoid reacting to daily noise.
</Callout>

## Common Pitfalls

- **Don't trade on daily score fluctuations** — conviction changes are meaningful, point changes are not. The composite score will move slightly every update cycle; wait for the conviction level to change before acting.
...
- **Don't chase conviction upgrades** — understand why the upgrade happened before acting. Open the asset detail page, review which factors improved, and assess whether the change is sustainable.
